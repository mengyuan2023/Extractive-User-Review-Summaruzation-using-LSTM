...
with graph.as_default():
    ...
    with sess.as_default():
        ...
        saver = tf.compat.v1.train.import_meta_graph('./runs/1565857877/checkpoints/model-512.meta')
        module_file = tf.train.latest_checkpoint("./runs/1565857877/" + 'checkpoints/')
        ...
        count = 0
        doc_empty_count = 0
        for xs, ys, zs in test_reader.iter():
            # count += 1
            # x = x[0]
            # y = y[0]
            #print (x)
            ys_ = sess.run(predict, feed_dict = {input_x: xs})
            ys_  = np.reshape(ys_, (batch_size, max_doc_length))
            flag = 0
            max_len = 0
            result = ''
            for x, y, y_, z in zip(xs, ys, ys_, zs):
                count += 1
                ...
                while len(tmp_str) < 10 and out_flag == 0:
                    ...
                    res[top_sentence_index] = cur_sentence
                    ...
                    if len(tmp_str) < 10:
                        if left < 0 and right > len(x) - 1:
                            ...
                            if len(x) == 41 and len(y_) == 16:
                                print(x)
                                print(y_)
                                for i in x:
                                    print(word_vocab.token(i))
                            if y_[right] >= y_[left]:
                                top_sentence_index = right
                                right += 1
                                # res.append(right)
                ...
                tmp_str += '\n'
                    # if out_flag == 0:
                    #     print(tmp_str)
                result = result + str(z)[1:-1] + '\t' + tmp_str
            f0.write(result)
            if count % 1000  == 0:
