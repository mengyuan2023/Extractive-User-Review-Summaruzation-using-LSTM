...
class SummaRuNNer(object):
    def __init__(self, vocabulary_size, embedding_size, batch_size, pretrained_embedding):
        ...
        with tf.variable_scope('inputs'):
            self.x = tf.placeholder(tf.int32, shape=[self.batch_size, self.doc_len, self.sent_len], name = "x_input")
            # self.x = tf.reshape(self.x, [-1, self.sent_len])
            self.y = tf.placeholder(tf.float32, shape=[self.batch_size, self.doc_len])
            # self.sequence_length = tf.reduce_sum(tf.sign(self.x), axis = 1)
            self.sequence_length = tf.reduce_sum(tf.sign(tf.reshape(self.x, [-1, self.sent_len])), axis=1)
        ...
        with tf.variable_scope('embedding_layer'):
            ...
            self.embed = tf.reshape(self.embed, [-1, self.sent_len, embedding_size])
        with tf.variable_scope("sent_level_BiGRU"):
            fw_cell = tf.nn.rnn_cell.GRUCell(self.hidden_size)
            fw_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.GRUCell(self.hidden_size) for i in range(3)], state_is_tuple=True)
            # fw_cell = tf.keras.layers.GRUCell(self.hidden_size)
            fw_cell = tf.contrib.rnn.DropoutWrapper(fw_cell, input_keep_prob=0.5)
            fw_cell_zero = fw_cell.zero_state(batch_size, dtype=tf.float32)
            bw_cell = tf.nn.rnn_cell.GRUCell(self.hidden_size)
            bw_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.GRUCell(self.hidden_size) for i in range(3)],
                                             state_is_tuple=True)
            # bw_cell = tf.keras.layers.GRUCell(self.hidden_size)
            bw_cell = tf.contrib.rnn.DropoutWrapper(bw_cell, input_keep_prob=0.5)
            # init_state = bw_cell.zero_state(batch_size, dtype=tf.float32)
            self.sent_GRU_out, _ = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, self.embed, scope='bi-GRU', dtype=tf.float32)
            # self.sent_GRU_out, _ = tf.keras.layers.Bidirectional(fw_cell, bw_cell, self.embed, scope='bi-GRU', dtype=tf.float32)
            self.outputs_1 = tf.concat([self.sent_GRU_out[0], self.sent_GRU_out[1]], 2)
            self.result = []
            # self.loca_out = self.outputs_1[:self.doc_length, :,:]
            # self.loca_out = self.outputs_1[:self.doc_length * batch_size, :, :]
        ...
        with tf.variable_scope("doc_level_BiGRU"):
            ...
            fw_cell_2 = tf.contrib.rnn.DropoutWrapper(fw_cell_2, input_keep_prob=0.5)
            bw_cell_2 = tf.nn.rnn_cell.GRUCell(self.hidden_size)
            bw_cell_2 = tf.contrib.rnn.DropoutWrapper(bw_cell_2, input_keep_prob=0.5)
            ...
            #position embedding
            Wpe = tf.Variable(tf.random_normal([batch_size * self.doc_len + 1, self.sent_len]))
            #dense layers 
        ...
        with tf.name_scope("loss"):
            ...
            target = self.y[:batch_size, :self.doc_length]
            target = tf.reshape(target, [-1])
            output = self.y_[:self.doc_length * batch_size]
